...
public final class ProjectCatalogRename {
  ...
  public static ai.singlr.sail.api.Operations.ProjectRenamed rename(Sqlite db, String from, String to) {
    ...
    return db.transaction(() -> {
      ...
      var existing = projects.findByName(from).orElseThrow(() ->
          new IllegalStateException("No project '" + from + "' in the catalog to rename."));
      ...
      return new ai.singlr.sail.api.Operations.ProjectRenamed(from, to, existing.definition());
    });
  }
  public static void restore(Sqlite db, ai.singlr.sail.api.Operations.ProjectRenamed renamed) {
    db.transaction(() -> {
