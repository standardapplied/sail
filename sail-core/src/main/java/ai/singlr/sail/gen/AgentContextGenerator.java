...
public final class AgentContextGenerator {
  ...
  /** Sail-owned core, regenerated every run; Claude imports it, Codex inlines a snapshot of it. */
  static final String SAIL_CONTEXT_PATH = ".sail/context.md";

  /**
   * Generates the full file set for the configured agents: the sail-owned {@code .sail/context.md}
   * core, one engineer-owned entry file per agent (CLAUDE.md / AGENTS.md), an engineer-owned
   * SECURITY.md, and the sail-owned methodology + spec-board skills. Empty when no agent is
   * configured.
   */
  public static List<GeneratedFile> generateFiles(SailYaml config) {
    ...
    var core = generateContextBody(config);
    files.add(new GeneratedFile(basePath + SAIL_CONTEXT_PATH, sailContext(core), false));

    for (var agent : targetAgents) {
      files.add(
          GeneratedFile.engineerOwned(basePath + agent.contextFileName(), entryFile(agent, core)));
    }

    files.add(GeneratedFile.engineerOwned(basePath + "SECURITY.md", generateSecurityMd(config)));
  ...
  /**
   * Generates the engineer-owned Claude entry file from the given project configuration.
   *
  ...
  public static String generateClaudeMd(SailYaml config) {
    return entryFile(AgentCli.CLAUDE_CODE, generateContextBody(config));
  }
  ...
  /** The sail-owned core file: a do-not-edit banner over the generated body. */
  static String sailContext(String core) {
    return "<!-- Generated by sail \u2014 regenerated on every run; do not edit, your changes will"
        + " be overwritten. Put personal notes in CLAUDE.md / AGENTS.md (yours) and project"
        + " config in sail.yaml. -->\n\n"
        + core;
  }

  /**
   * The engineer-owned entry file sail writes once. Claude Code can {@code @}-import the sail core,
   * so its file stays a thin pointer that always sees fresh content; Codex has no import (verified
   * against the Codex docs), so its file inlines a snapshot of the core that {@code regen --force}
   * refreshes.
   */
  static String entryFile(AgentCli agent, String core) {
    var ownershipNote =
        "<!-- This file is yours. Sail wrote it once and will not touch it again \u2014 edit it"
            + " freely. -->\n";
    return switch (agent) {
      case CLAUDE_CODE ->
          "# CLAUDE.md\n\n"
      ...
      case CODEX ->
          "# AGENTS.md\n\n"
              + ownershipNote
              + "<!-- The project context below is a snapshot from sail; run `sail agent context"
              + " regen --force` to refresh it. -->\n\n"
              + core
              + "\nReview SECURITY.md for security requirements.\n";
    };
  }
   ...
   * rule one engineer adds reaches the whole team). Engineer-owned: sail scaffolds it once and
   * leaves it alone thereafter; {@code regen --force} resets it to this generated template.
   */
  static String generateSecurityMd(SailYaml config) {
    var baseline =
        """
        # SECURITY.md \u2014 Security Requirements and Audit Checklist
        # Sail scaffolded this file once from sail.yaml; it is yours \u2014 edit it freely. Sail will
        # not overwrite it again unless you run `sail agent context regen --force`. Project-wide
        # security rules belong in sail.yaml (agent_context.security) so they reach the whole team.
